...
class LanguageManager
{
	...
	{
		save = new FlxSave();
		save.bind('languageFoxa', 'FoxaTheArtist');
	}
	...
	public static function init()
	{
		LanguageManager.currentLocaleList = CoolUtil.coolTextFile(Paths.file('locale/' + LanguageManager.save.data.language + '/textList.txt', TEXT,
			'preload'));
		LanguageManager.currentTerminalList = CoolUtil.coolTextFile(Paths.file('locale/' + LanguageManager.save.data.language + '/terminalList.txt', TEXT,
			'preload'));
	}
